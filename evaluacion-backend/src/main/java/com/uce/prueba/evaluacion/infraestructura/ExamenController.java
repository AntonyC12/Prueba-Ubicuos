...
public class ExamenController {
    ...
    public ExamenController(RealizarExamen realizarExamen, NotaRepository notaRepository) {
    ...
    public ResponseEntity<Map<String, Object>> obtenerResultado(@PathVariable Long evaluadoId) {
        ...
        if (notaOpt.isEmpty()) {
            response.put("estado", "PROCESANDO");
            response.put("mensaje", "El diagnóstico aún se está procesando...");
        ...
        response.put(
                "resultado",
                nota.getPuntaje() + "/" + nota.getTotal()
                + " (" + String.format("%.1f", nota.getPorcentaje()) + "%)"
        );
        response.put("nota", nota.getValor());
        response.put("nivel", nota.getNivel().toString());
    ...
    public static class RespuestaRequestItem {
        ...
        public String getReactivoId() {
            return reactivoId;
        }

        public void setReactivoId(String reactivoId) {
